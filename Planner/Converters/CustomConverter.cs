...
namespace Planner.Converters
{
    ...
    /// </summary>
    public class CustomConverter<T> : Converter<T>
    {
        ...
        protected virtual T? ConvertFromString(string? value)
        {
            try
            {
                ...
                else if (typeof(T) == typeof(bool) || typeof(T) == typeof(bool?))
                {
                    ...
                    UpdateGetError("Недопустимое логическое значение");
                }
                // sbyte
                else if (typeof(T) == typeof(sbyte) || typeof(T) == typeof(sbyte?))
                {
                    ...
                    UpdateGetError("Недействительный номер");
                }
                // byte
                ...
                else if (typeof(T) == typeof(Guid) || typeof(T) == typeof(Guid?))
                {
                    if (Guid.TryParse(value, out var parsedValue))
                        return (T)(object)parsedValue;
                    UpdateGetError("Недействительный GUID");
                }
                // enum
                else if (IsNullableEnum(typeof(T?)))
                {
                    ...
                    UpdateGetError("Не значение " + enum_type?.Name);
                }
                else if (typeof(T).IsEnum)
                {
                    if (Enum.TryParse(typeof(T), value, out var parsedValue))
                        return (T)parsedValue;
                    UpdateGetError("Не значение " + typeof(T).Name);
                }
                // datetime
                else if (typeof(T) == typeof(DateTime) || typeof(T) == typeof(DateTime?))
                {
                    ...
                    {
                        UpdateGetError("Недопустимая дата и время.");
                    }
                }
                // timespan
                else if (typeof(T) == typeof(TimeSpan) || typeof(T) == typeof(TimeSpan?))
                {
                    ...
                    {
                        UpdateGetError("Недопустимый интервал времени");
                    }
                }
                else
                {
                    UpdateGetError($"Преобразование в тип {typeof(T)} не реализована");
                }
            }
            catch (Exception e)
            {
                UpdateGetError("Ошибка конвертации: " + e.Message);
            }
        ...
        protected virtual string? ConvertToString(T? arg)
        {
            ...
            {
                UpdateSetError("Ошибка конвертации: " + e.Message);
                return null;
            }
        }
